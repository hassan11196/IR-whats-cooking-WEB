...
export default function Home(props: Props) {
  ...
  const flagRenderer = (item) => {
    return <Flag name={item} />;
  };
  const getData = () => {
    if (flag === false) {
      axios
        .get("/api/ingredients?limit=6000&offset=0")
        .then((response) => {
    ...
    const res = axios
      ...
      .then((response) => {
        ...
        setAlert({
          status: true,
          title: "Prediction",
          text: `Cuisine: ${response.data.result}`,
        });
        return response;
      })
  ...
  return (
    <>
      <Menu stackable inverted pointing secondary>
        <Menu.Item>IR - AI Project</Menu.Item>
                ...
                <Row>
                  <Col md="7">
                    <Input
                      ...
                      icon="search"
                      placeholder="Click on item to select..."
                      onChange={onSearch}
                    />

                    <Card.Content
                      style={{ marginTop: "1rem", height: "34rem" }}
                    >
                      <List divided relaxed>
                        {copyList.map((item, index) => {
                          if (index <= lastIndex && index >= prevIndex)
                            return (
                              <List.Item
                                name={item}
                                key={index}
                    ...
                    <Card.Content
                      extra
                      style={{ float: "bottom", marginTop: "1rem" }}
                    >
                      <div
                        style={{
                          display: "flex",
                              ...
                              return (
                                ...
                                >
                                  <Card
                                    name={item}
                                    onClick={(e, item) => {
                                      handleCreate2(e, item);
                                    }}
                                  >
                                    <Card.Content
                                      style={{
                                        backgroundColor:
                                          GoogleColors[index % 5],
                                        color: "black",
                                        fontWeight: "700",
                                        textAlign: "center ",
                                      }}
